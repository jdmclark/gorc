...
test_case(throws)
{
    ...
    expect_throws(throw std::runtime_error("foo"), std::runtime_error, "foo");

    test::test_reporter = originalReporter;

    assert_true(test::test_passing);

    assert_eq(mockReporter->case_expectation_fail_called, 0);
}
...
test_case(no_throw)
{
    ...
    expect_throws(scratch(), std::runtime_error, "foo");
...
test_case(throw_mismatch)
{
    ...
    expect_throws(throw std::runtime_error("bar"), std::runtime_error, "foo");
...
test_case(throw_wrong_type)
{
    ...
    expect_throws(throw std::domain_error("foo"), std::runtime_error, "foo");
