...
namespace {
    class bm_pal_context {
    public:
        gorc::content::assets::colormap const *pal_colormap = nullptr;
        gorc::content::assets::bitmap const *pal_bitmap = nullptr;

        gorc::vector<3, uint8_t> get_color(uint8_t index) const
        {
            if(pal_colormap) {
            ...
            return gorc::make_zero_vector<3, uint8_t>();
        }
    };
}

std::unique_ptr<gorc::asset> gorc::content::loaders::bitmap_loader::deserialize(
        input_stream& file, content_manager&, service_registry const &) const {
    bm_pal_context pal;
    pal.pal_bitmap = pal_bitmap;
    pal.pal_colormap = pal_colormap;
    ...
    if(strncmp(magic, "BM  ", 4) != 0) {
        LOG_FATAL("file is not a valid BM");
    }
    ...
    if(header.pal_included == 2) {
        ...
        pal.pal_bitmap = mat.get();
        pal.pal_colormap = nullptr;
    }
    ...
    if(header.bit_depth == 8) {
        ...
        for(auto& img : images) {
            ...
            for(int i = 0, j = 0; i < num_pels; j += stride) {
                for(int k = 0; k < std::get<0>(img); ++k, ++i, j += 4) {
                    ...
                    auto pal_col = pal.get_color(pal_idx);
        ...
        // Unknown bit depth
        LOG_FATAL(format("bitdepth %d is not supported") % header.bit_depth);
    }
