...
symbol_visitor::symbol_visitor(symbols::symbol_table& symbolTable,
    ...
    : symbol_table(symbolTable), SeenLabels(SeenLabels) {
    return;
}

gorc::cog::vm::value symbol_visitor::get_value(symbol &symbol) {
    symbols::symbol_type type = get_type(symbol);

    if(symbol.value.has_value()) {
        symbol_field *field = symbol.value.get_value();
        ...
        case symbols::symbol_type::thing_template:
            if(!v.is_str) {
                ...
                return vm::value(symbol_table.add_string(v.str_value));
            }
            break;
...
void symbol_visitor::visit(symbol& symbol) {
    symbols::symbol_type type = get_type(symbol);

    vm::value defaultvalue = get_value(symbol);
    ...
    if(symbol_table.is_symbol_defined(symbol.name)) {
        LOG_WARNING(format("symbol '%s' redefinition") % symbol.name);
        ...
        if(type == symbols::symbol_type::message) {
            if(SeenLabels.find(symbol.name) == SeenLabels.end()) {
                LOG_WARNING(format("message '%s' was exported but label not defined") % symbol.name);
            }
        }
    }
}
...
gorc::cog::symbols::symbol_type symbol_visitor::get_type(symbol& symbol) {
    ...
    if(symbol.type == "ai") {
        ...
        LOG_ERROR(format("unknown type '%s'") % symbol.type);
        return symbols::symbol_type::integer;
    }
}
