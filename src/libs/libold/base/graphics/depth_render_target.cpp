#include "depth_render_target.hpp"
#include <iostream>
...
gorc::graphics::depth_render_target::depth_render_target(const box<2, int>& size) : render_target(size) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, get_size<0>(size), get_size<1>(size), 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
