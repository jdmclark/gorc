...
#include "mouse_click_dispatcher.hpp"
#include "keyboard_key_dispatcher.hpp"
#include <array>
...
public:
    ...
    virtual void set_mouse_cursor_position(const gorc::time& time, const vector<2, int>& cursor_pos) {
    ...
    virtual void handle_mouse_input(const gorc::time& time) {
        if(view.wants_mouse_focus()) {
            click_dispatcher.handle_mouse_input(time);
        }
    }

    virtual void handle_keyboard_input(const gorc::time& time) {
        if(view.wants_keyboard_focus()) {
            key_dispatcher.handle_keyboard_input(time);
        }
    }

    virtual void handle_text_input(const gorc::time& time, char ch) {
        if(view.wants_keyboard_focus()) {
