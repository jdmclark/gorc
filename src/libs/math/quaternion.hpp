...
template <typename G> quaternion<G> make_rotation(const vector<3, G>& axis, G angle) {
    auto s = static_cast<G>(sin(to_radians(angle) / G(2)));
    return quaternion<G>(make_vector(
            static_cast<G>(cos(to_radians(angle) / G(2))),
            get<0>(axis) * s,
            get<1>(axis) * s,
            get<2>(axis) * s));
}
...
template <typename G> quaternion<G> slerp(const quaternion<G>& a, const quaternion<G>& b, G alpha) {
    ...
    double ratio_a = sin((1.0 - static_cast<double>(alpha)) * half_theta) / sin_half_theta;
    double ratio_b = sin(static_cast<double>(alpha) * half_theta) / sin_half_theta;

    return quaternion<G>(static_cast<vector<4, double>>(mod_a.q) * ratio_a +
                         static_cast<vector<4, double>>(b.q) * ratio_b);
}
