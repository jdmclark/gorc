...
gorc::content::vfs::virtual_file_system::vfs_episode::vfs_episode(io::read_only_file& file, std::unique_ptr<class container>& cnt, diagnostics::report& report)
    : cntr(std::move(cnt)) {
...
const gorc::content::vfs::episode::episode& gorc::content::vfs::virtual_file_system::vfs_episode::get_episode() const {
    return *Episode;
}

vfsmap::const_iterator gorc::content::vfs::virtual_file_system::vfs_episode::find(const std::string& path) const {
    return FileMap.find(path);
}

vfsmap::const_iterator gorc::content::vfs::virtual_file_system::vfs_episode::begin() const {
    return FileMap.begin();
}

vfsmap::const_iterator gorc::content::vfs::virtual_file_system::vfs_episode::end() const {
    return FileMap.end();
}

gorc::content::vfs::virtual_file_system::virtual_file_system(const boost::filesystem::path& RestrictedPath, const boost::filesystem::path& ResourcePath,
...
void gorc::content::vfs::virtual_file_system::loadGob(const boost::filesystem::path& gobFilePath, vfsmap& map, diagnostics::report& report) {
    std::unique_ptr<container> cnt;

    try {
        io::native_file file(gobFilePath, false);
        cnt = std::unique_ptr<container>(new gob::gob_container(gobFilePath, file));
    }
...
void gorc::content::vfs::virtual_file_system::loadGobs(const boost::filesystem::path& basepath, vfsmap& map, diagnostics::report& report) {
...
std::unique_ptr<gorc::io::read_only_file> gorc::content::vfs::virtual_file_system::findInGobs(const boost::filesystem::path& path,
...
std::unique_ptr<gorc::io::read_only_file> gorc::content::vfs::virtual_file_system::open(const boost::filesystem::path& path) const {
...
void gorc::content::vfs::virtual_file_system::list(std::ostream& os) const {
