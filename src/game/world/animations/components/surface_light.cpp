...
gorc::game::world::animations::components::surface_light::surface_light(entity_id surface, float start_light, float end_light, float change_time)
    : surface(surface), start_light(start_light), end_light(end_light), change_time(change_time), anim_time(0.0f) {
    return;
}

gorc::game::world::animations::components::surface_light::surface_light(io::deserialization_constructor_tag tag, io::binary_input_stream& is)
    : surface(tag, is) {
