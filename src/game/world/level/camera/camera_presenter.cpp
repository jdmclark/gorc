...
gorc::game::world::level::camera::camera_presenter::camera_presenter(level_presenter& presenter)
	: presenter(presenter), levelmodel(nullptr), model(nullptr) {
	return;
}

void gorc::game::world::level::camera::camera_presenter::start(level_model& levelmodel, camera_model& model) {
...
void gorc::game::world::level::camera::camera_presenter::update(double dt) {
...
void gorc::game::world::level::camera::camera_presenter::cycle_camera() {
	set_current_camera((model->current_camera + 1) % 2);
}

int gorc::game::world::level::camera::camera_presenter::get_camera_state_flags() {
	return static_cast<int>(model->state_flags);
}

int gorc::game::world::level::camera::camera_presenter::get_current_camera() {
	return model->current_camera;
}

int gorc::game::world::level::camera::camera_presenter::get_primary_focus(int camera_id) {
	return model->cameras[camera_id].focus;
}

void gorc::game::world::level::camera::camera_presenter::set_camera_focus(int camera_id, int focus_object_id) {
	model->cameras[camera_id].focus = focus_object_id;
}

void gorc::game::world::level::camera::camera_presenter::set_camera_state_flags(int flags) {
	model->state_flags = flag_set<flags::camera_state_flag>(flags);
}

void gorc::game::world::level::camera::camera_presenter::set_current_camera(int camera_id) {
	model->current_camera = camera_id;
}

void gorc::game::world::level::camera::camera_presenter::set_pov_shake(const vector<3>& pos_offset, const vector<3>& ang_offset, float pos_reset_speed, float ang_reset_speed) {
...
void gorc::game::world::level::camera::camera_presenter::register_verbs(cog::verbs::verb_table& verbTable, components& components) {
