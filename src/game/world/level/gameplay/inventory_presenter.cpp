...
gorc::game::world::level::gameplay::inventory_presenter::inventory_presenter(level_presenter& presenter)
	: presenter(presenter) {
	return;
}

void gorc::game::world::level::gameplay::inventory_presenter::start(level_model& levelModel, inventory_model& model) {
	this->model = &model;
	this->levelModel = &levelModel;
	return;
}

void gorc::game::world::level::gameplay::inventory_presenter::update(double dt) {
	model->mod_all_cooldowns(-dt);
}

void gorc::game::world::level::gameplay::inventory_presenter::change_inv(int player, int bin, int amount) {
	model->get_inventory(player).mod_bin_value(bin, amount);
}

int gorc::game::world::level::gameplay::inventory_presenter::get_inv(int player, int bin) {
	return model->get_inventory(player).get_bin_value(bin);
}

int gorc::game::world::level::gameplay::inventory_presenter::get_inv_cog(int player, int bin) {
	const content::assets::inventory_bin& inv_bin = model->base_inventory.get_bin(bin);
	if(inv_bin.cog) {
		return presenter.script_presenter.get_global_cog_instance(&inv_bin.cog->script);
	}
	else {
		return -1;
	}
...
int gorc::game::world::level::gameplay::inventory_presenter::get_inv_max(int player, int bin) {
	return model->base_inventory.get_bin(bin).max_value;
}

int gorc::game::world::level::gameplay::inventory_presenter::get_inv_min(int player, int bin) {
	return model->base_inventory.get_bin(bin).min_value;
}

bool gorc::game::world::level::gameplay::inventory_presenter::is_inv_activated(int player, int bin) {
	return model->get_inventory(player).is_bin_activated(bin);
}

bool gorc::game::world::level::gameplay::inventory_presenter::is_inv_available(int player, int bin) {
	return model->get_inventory(player).is_bin_available(bin);
}

void gorc::game::world::level::gameplay::inventory_presenter::set_bin_wait(int player, int bin, float delay) {
	model->get_inventory(player).set_bin_cooldown(bin, delay);
}

void gorc::game::world::level::gameplay::inventory_presenter::set_inv(int player, int bin, int value) {
	model->get_inventory(player).set_bin_value(bin, value);
}

void gorc::game::world::level::gameplay::inventory_presenter::set_inv_activated(int player, int bin, bool value) {
	model->get_inventory(player).set_bin_activated(bin, value);
}

void gorc::game::world::level::gameplay::inventory_presenter::set_inv_available(int player, int bin, bool value) {
	model->get_inventory(player).set_bin_available(bin, value);
}

void gorc::game::world::level::gameplay::inventory_presenter::on_item_hotkey_pressed(int player, int bin) {
...
void gorc::game::world::level::gameplay::inventory_presenter::on_item_hotkey_released(int player, int bin) {
	// TODO: If weapon, change 'bin' to 0 for primary, 1 for secondary fire.
...
void gorc::game::world::level::gameplay::inventory_presenter::register_verbs(cog::verbs::verb_table& verbTable, components& components) {
