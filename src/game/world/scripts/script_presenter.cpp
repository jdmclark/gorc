...
void gorc::game::world::scripts::script_presenter::create_level_cog_instance(int index, const cog::script& script, content_manager& manager,
        cog::compiler&, const std::vector<cog::vm::value>& values) {
    auto& cog_inst_pair = model->cogs[index];
    std::get<0>(cog_inst_pair) = std::make_unique<cog::instance>(script);
...
void gorc::game::world::scripts::script_presenter::create_global_cog_instance(const cog::script& script,
        content_manager& manager, cog::compiler&) {
    if(model->global_script_instances.find(&script) != model->global_script_instances.end()) {
        // Instance already created.
        return;
    }

    model->cogs.emplace_back(std::make_unique<cog::instance>(script), cog::scripts::script_timer_state());
...
void gorc::game::world::scripts::script_presenter::send_message_to_linked(cog::message_id message,
    ...
    maybe<size_t> capture_cog = nothing;
    maybe<size_t> class_cog = nothing;
